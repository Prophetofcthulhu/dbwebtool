(function () {
    ...
    window.$tablerender = {
        ...
        data_update: function(name, data, datapointname) {
            ...
            if ($$.is_defined(data.meta)) {
                ...
                view.meta = data.meta;
                //this.update_dynamic_model(name, view, model, types);
                this.update_meta_model(name);
                // view = this.child_view(name, data.meta);
                // name = view.name;
                // console.log("Child view [" + name + "]")
            }
        ...
        ___start_external: function(name, action_name, key_name) {
            var self = this,
                view = this.views[name],
                actions = view.actions || {},
                action = actions[action_name];
            ...
            // todo - refactor params set up
            var interesting = action.interesting,
                url = $$.is_defined(key_name) ? action.url + key_name + "/" : action.url,
            ...
            data = data_collect(),
                action_url = url + "?data=" + JSON.stringify(data);

            //console.log("ACTION URL: [" + action_url + "] Performer: [" + performer + "]");
            performer = window[performer];
            performer(action_prefix + action_name, action_url, action_params[0], action_params[1]);
        },
        update_meta_model: function(name) {
            var view = this.views[name],
                meta = view.meta;
            ...
            for (var i=0; i<model.length; i+=1) {
                var field_name = model[i],
                    field_info = data_types[i],
                    field_type = field_info.type,
                    sortable_types = view.sortable_types,
                    searchable_types = view.searchable_types,
                    title_field_template = titles[field_type],
                    data_field_template = types[field_type],
                    sortable = $$.contains_string(sortable_types, field_type),
                    searchable = $$.contains_string(searchable_types, field_type);
    ...
    hide_column: function(name, field_name) {
            var view = this.views[name],
                meta = view.meta,
                whole_meta = $$.is_defined(view.whole_meta) ? view.whole_meta : $$.deep_copy(meta),
                model = meta.model,
                data_types = meta.types,
                index = $$.index_of(model, field_name);

                model.splice(index, 1);
                data_types.splice(index, 1);

            view.whole_meta = whole_meta;

            this.update_meta_model(name);
            this._render(name);

            // console.log("Column [" + field_name + "] hide")
    },
    move_column: function(name, field_name, direction) {
            var view = this.views[name],
                ...
                original_position = $$.index_of(model, field_name),
                new_position = original_position + dir;

            meta.model = $$.shift_element(model, original_position, dir);
            meta.types = $$.shift_element(data_types, original_position, dir);

            this.update_meta_model(name);
            this._render(name);

            console.log("Column [" + field_name + "] moved " + direction
    ...
    init_config: function(kind, config) {
        ...
        update_columns_number: function (name, delta) {
            var view = this.views[name],
                stretched_columns = view.stretched_columns,
                colspan = "colspan";
        ...
        columns_number_reconciliation: function(name) {
            var view = this.views[name],
                actual_columns_number = parseInt(view.meta.model.length),
